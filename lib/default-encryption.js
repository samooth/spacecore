...
module.exports = class DefaultEncryption {
  ...
  constructor (encryptionKey, spacecoreKey, opts = {}) {
    this.key = encryptionKey
    this.compat = opts.compat === true

    const keys = DefaultEncryption.deriveKeys(encryptionKey, spacecoreKey, opts)

    this.blockKey = keys.block
    this.blindingKey = keys.blinding
  }

  static deriveKeys (encryptionKey, spacecoreKey, { block = false, compat = false } = {}) {
    ...
    if (!block) {
      if (compat) sodium.crypto_generichash_batch(blockKey, [encryptionKey], spacecoreKey)
      else sodium.crypto_generichash_batch(blockKey, [DEFAULT_ENCRYPTION, spacecoreKey, encryptionKey])
    }
  ...
  static blockEncryptionKey (spacecoreKey, encryptionKey) {
    const blockKey = b4a.alloc(sodium.crypto_stream_KEYBYTES)
    sodium.crypto_generichash_batch(blockKey, [DEFAULT_ENCRYPTION, spacecoreKey, encryptionKey])
    return blockKey
  }
