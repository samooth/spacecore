...
const { BAD_ARGUMENT, STORAGE_EMPTY, STORAGE_CONFLICT, INVALID_SIGNATURE, INVALID_CHECKSUM } = require('../../spacecore-errors')
...
module.exports = class Core {
  ...
  async _tryOpen (opts) {
    ...
    if (!header && (opts.discoveryKey && !(opts.key || opts.manifest))) {
      throw STORAGE_EMPTY('No Spacecore is stored here')
    }
    ...
    if (opts.key && !b4a.equals(header.key, opts.key)) {
      throw STORAGE_CONFLICT('Another Spacecore is stored here')
    }
  ...
  async checkConflict (proof, from) {
    ...
    console.log('[spacecore] conflict detected in ' + id + ' (writable=' + !!this.header.keyPair + ',quorum=' + this.header.manifest.quorum + ')')
