/* DEV DOCS
  Every spacecore has one Replicator object managing its connections to other peers.
  There is one Peer object per peer connected to the Spacecore.
  Spacecores do not know about other spacecores, so when a peer is connected to multiple cores, there exists one Peer object per core.

  Spacecore indicates block should be downloaded through methods like Replicator.addRange or Replicator.addBlock
  Spacecore calls Replicator.updateActivity every time a spacecore session opens/closes
  Replicator.updateActivity ensures the Spacecore is downloading blocks as expected
...
const { REQUEST_CANCELLED, REQUEST_TIMEOUT, INVALID_CAPABILITY, SNAPSHOT_NOT_AVAILABLE } = require('../../spacecore-errors')
...
module.exports = class Replicator {
  ...
  setDownloadingNow (downloading) {
    ...
    if (downloading) { // restart channel if needed...
      for (const protomux of this._attached) {
        ...
        if (protomux.opened({ protocol: 'spacecore/alpha', id: this.core.discoveryKey })) continue
  ...
  attachTo (protomux) {
    ...
    protomux.pair({ protocol: 'spacecore/alpha', id: this.core.discoveryKey }, makePeer)
  ...
  detachFrom (protomux) {
    if (this._attached.delete(protomux)) {
      ...
      protomux.unpair({ protocol: 'spacecore/alpha', id: this.core.discoveryKey })
    }
  }
  ...
  _makePeer (protomux) {
    ...
    if (protomux.opened({ protocol: 'spacecore/alpha', id: this.core.discoveryKey })) return onnochannel()

    const channel = protomux.createChannel({
      userData: null,
      protocol: 'spacecore/alpha',
      aliases: ['spacecore'],
...
function isCriticalError (err) {
  // TODO: expose .critical or similar on the spacecore errors that are critical (if all not are)
  return err.name === 'SpacecoreError'
}
